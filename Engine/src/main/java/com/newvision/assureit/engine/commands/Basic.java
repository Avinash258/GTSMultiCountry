...
public class Basic extends General {
    ...
    public void TakeMobileScreenshot() {
        if (elementEnabled()) {
            try {
                ...
                Report.updateTestLog(Action, "Screenshot taken for " + ObjectName, Status.DONE, destinationFile.getAbsolutePath());
            } catch (IOException e) {
    ...
    @Action(object = ObjectType.MOBILE, desc = "Retrieve device information.")
    ...
    @Action(object = ObjectType.MOBILE, desc = "Retrieve text content from an element.", input = InputType.NO)
    public void GetText() {
	if (elementEnabled()) {
	    String email = Element.getAttribute("text"); // Retrieve the text from the input field
	    Report.updateTestLog(Action, "Retrieved email '" + email + "' from '"
		    + ObjectName + "'", Status.DONE);
	} else {
	    throw new ElementException(ExceptionType.Element_Not_Enabled, ObjectName);
	}
    ...
    @Action(object = ObjectType.MOBILE, desc = "Delay execution for a specified period.", input = InputType.YES)
    public void Wait() {
	try {
    ...
    try {
        ...
        if (Driver instanceof AndroidDriver) {
            ...
            String apkPath = Data;  // Retrieve the APK path from user input
        ...
        try {
            if (Driver instanceof AndroidDriver) {
                ...
                switch (Data.toUpperCase()) {
                    case "AIRPLANE":
                        command = "adb shell settings put global airplane_mode_on 1 && adb shell am broadcast -a android.intent.action.AIRPLANE_MODE --ez state true";
                        break;
                    case "WIFI":
                ...
                Report.updateTestLog(Action, "Network state set to " + Data, Status.DONE);
            } else {
                throw new UnsupportedOperationException("Driver is not an instance of AndroidDriver");
            }
        } catch (Exception e) {
...
public void TapAppElement() {
    ...
    @Action(object = ObjectType.MOBILE, desc = "Handle alert by accepting, dismissing, or entering text based on the input.", input = InputType.YES)
    ...
    @Action(object = ObjectType.MOBILE, desc = "Record the mobile screen.")
    public void RecordMobileScreen() {
    if (Driver instanceof AndroidDriver) {
        try {
                AndroidDriver androidDriver = (AndroidDriver) Driver;
            ...
            // Add a sleep or wait here to record the screen for a specific duration
            // For example, to record for 10 seconds
            Thread.sleep(10000);
 
            // Stop screen recording and get the video as a Base64 string
            String base64String = androidDriver.stopRecordingScreen();
 
            // Get base path from system property or environment variable
            String basePath = System.getProperty("recording.path");
            if (basePath == null) {
                basePath = System.getenv("RECORDING_PATH");
                if (basePath == null) {
                    basePath = System.getProperty("user.home") + "/path/to/"; // Fallback to user home directory
                }
            }
            String filePath = basePath + "recording_" + ObjectName + ".mp4";
 
            // Specify the path where the video file will be saved
            File videoFile = new File(filePath);
 
            // Ensure the directory exists
            if (!videoFile.getParentFile().exists() && !videoFile.getParentFile().mkdirs()) {
                throw new IOException("Failed to create directory: " + videoFile.getParentFile().getAbsolutePath());
            }
            ...
            // Update the report with the video file path
            Report.updateTestLog(Action, "Screen recording saved for " + ObjectName, Status.DONE, videoFile.getAbsolutePath());
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            Report.updateTestLog(Action, "Screen recording interrupted for " + ObjectName, Status.FAIL, e.getMessage());
        } catch (IOException e) {
