...
public class Webservice extends General {
   ...
   private void createPatchHttprequest() {
    ...
    private void savePayload(String reqOrRes, String data) {
       ...
       @Action(object = ObjectType.WEBSERVICE, desc = "Assert Response Code ", input = InputType.YES,condition = InputType.OPTIONAL)
    public void assertResponseCodeLoopTime() {
     int t = Integer.parseInt(Condition);
        try {
            if (responsecodes.get(key).equals(Data)) {
                Report.updateTestLog(Action, "Status code is : " + Data, Status.PASSNS);
            } else {
